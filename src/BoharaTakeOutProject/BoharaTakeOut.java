package BoharaTakeOutProject;

import javax.print.attribute.standard.MediaSizeName;
import java.util.Scanner;

public class BoharaTakeOut {
	...
	public static void main(String[] args) {
		...
		do {
			...
			case"1":
				BoharaTakeOut.displayItalianFoodMenu();
			int[]feedItalianFood = {5, 7, 4};
			double[]priceItalian = {17.99, 15.99, 12.99};
			int opt = BoharaTakeOut.foodChoice(feedItalianFood, priceItalian);
			
			BoharaTakeOut.finalPrice(feedItalianFood, priceItalian, opt, numberOfPeople);
			isWrongMenuChoice = false;
			break;
			
			case "2":
				BoharaTakeOut.displayChineseFoodMenu();
				int[]feedChineseFood = {7, 7, 5};
				double[]priceChinese = {18.99, 18.99, 10.99};
				opt = BoharaTakeOut.foodChoice(feedChineseFood, priceChinese);
				BoharaTakeOut.finalPrice(feedChineseFood, priceChinese, opt, numberOfPeople);
				isWrongMenuChoice = false;
				break;
				
				case "3":
					BoharaTakeOut.displayAmericanFoodMenu();
				int[]feedAmericanFood = {8, 5, 10};
				double[]priceAmerican = {21.99, 22.99, 26.99};
				opt = BoharaTakeOut.foodChoice(feedAmericanFood, priceAmerican);
				BoharaTakeOut.finalPrice(feedAmericanFood, priceAmerican, opt, numberOfPeople);
				isWrongMenuChoice = false;
				break;
	...
	public static void displayChineseFoodMenu() {
		String[] chineseFood = { "Chicken and Broccoli Tray (includes 7 wontonsoups, 7 egg rolls)",
				"Sweet and Sour Pork Tray(includes 7 hot and sour soups, 7 egg rolls)",
				"Shrim Fried Rice Tray(includes 10 egg rolls)" };
		System.out.println("------------");
...
//Display food trays menu
	...
	public static void finalPrice(int[] feeds, double[] prices, int choice, int guestCount) {
		int trays = BoharaTakeOut.determineTrays(guestCount, feeds[choice - 1]);
		double subTotal = BoharaTakeOut.getSubtotal(prices[choice - 1], trays);
		double tax = BoharaTakeOut.getTax(subTotal, .07);
		double tip = BoharaTakeOut.getTip(subTotal, .15);
		double grandTotal = BoharaTakeOut.getGrandTotal(subTotal, tax, tip);
		...
		System.out.printf("prices per person: $ % .2f\n", BoharaTakeOut.pricePerPerson(grandTotal, guestCount));
		System.out.println("Left-over servings for the delivery person: "
				+ BoharaTakeOut.determineLeftOvers(feeds[choice - 1], trays, guestCount));
